package com.whb.redisson.demoTwo.uuid;

import java.text.SimpleDateFormat;
import java.util.Date;

import com.whb.redisson.demoTwo.manager.RedisPoolAPIManager;
import com.whb.redisson.demoTwo.util.DistributedLockUtil;
...
public class UUidGenerator {
	...
	public String getNextUuid(String preKey, int cacheNum) {
		return getNextUuid(preKey, this.defaultTotalNum - this.preKeyLen, "0", substractPreKey(preKey) + "#", true,
				cacheNum);
	}
	...
	public String getNextUuid(String preKey, int length, String fillChar, String formatStr, boolean needFormat,
			int cacheNum) {
		String uuidStr = "";
		...
		try {
			...
			try {
				...
				if ((nowUuid == null) || (nowUuid.intValue() == 0)) {
					...
					if (Integer.valueOf(jedis.get(REDIS_DB + preKey)) <= Integer
							.valueOf(jedis.get(REDIS_CACH + preKey))) {
				...
				System.out.println("线程 redis :" + Thread.currentThread().getId() + " exception :"
			...
			System.out.println("线程锁 :" + Thread.currentThread().getId() + " exception :"
					+ new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()));
			e.printStackTrace();
		} finally {
